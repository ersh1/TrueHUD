...
namespace Messaging
{
	...
	void TrueHUDInterface::DrawArc(const RE::NiPoint3& a_origin, float a_radius, float a_startAngle, float a_endAngle, const RE::NiMatrix3& a_matrix, uint32_t a_segments, float a_duration, uint32_t a_color, float a_thickness) noexcept
	{
		auto hudHandler = HUDHandler::GetSingleton();
		hudHandler->DrawArc(a_origin, a_radius, a_startAngle, a_endAngle, a_matrix, a_segments, a_duration, a_color, a_thickness);
	}
